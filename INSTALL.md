...
Для обычного обновления кода запустите `update.sh` из папки, из которой
ставили:
...
Или одной командой со своего компьютера:

```bash
ssh root@IP_СЕРВЕРА 'bash ~/market_yookassa/update.sh'
```

Скрипт сам делает `git pull`. Если репозиторий закрытый и код
копируется rsync'ом (см. [выше](#если-репозиторий-закрытый)), скопируйте
изменения тем же способом и запустите `update.sh` — без `.git` он берёт
папку как есть. Если ставили через `curl`, скрипт лежит в
`/usr/local/src/market-yookassa`.

`update.sh`:

- копирует новый код в `/opt/market-yookassa`, не трогая `.env`,
  `uploads/` и загруженные картинки;
- ставит зависимости, только если изменился `package-lock.json`;
- применяет изменения схемы БД;
- пересобирает и перезапускает приложение.

Пакеты, nginx, сертификат, файрвол, `.env`, службы systemd и
администратора он не трогает. Если в новой версии изменилась сама
установка (конфигурация nginx или служб в `install.sh`), запустите
полное обновление: `sudo bash install.sh` — повторный запуск тоже
безопасен.
