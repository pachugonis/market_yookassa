# 🛍️ Amazonus - Маркетплейс цифровых товаров
...
Переключатель в «Админка → Настройки → Режим одного продавца» превращает
площадку в магазин одного лица:

- регистрация с ролью продавца закрыта — новые аккаунты создаются только
  покупателями (роль `SELLER` при необходимости выдаётся вручную в
  разделе «Пользователи»);
- создавать и править товары может только администратор, даже если роль
  `SELLER` осталась у аккаунтов, заведённых до включения режима;
- сплитования нет: вся сумма сделки приходит на счёт площадки, реквизиты
  продавцов у платёжных сервисов не используются, привязка карты для
  выплат отключена.

Кабинет продавца при этом остаётся доступным: там история продаж и
остаток на балансе, который прежние продавцы смогут вывести.
...
Для отправки email уведомлений необходимо настроить SMTP:

### Через панель администратора

1. Перейдите в **Админ панель** → **Настройки** → **Настройки Email**
2. Заполните SMTP данные:
   - **SMTP Host**: например `smtp.gmail.com`, `smtp.mail.ru`, или `smtp.yandex.ru`
   - **SMTP Port**: обычно `587` (TLS) или `465` (SSL)
   - **SMTP User**: ваш email адрес
   - **SMTP Password**: пароль или пароль приложения
   - **Отправитель Email**: email для отправки писем
   - **Имя отправителя**: имя, которое будет видно получателям
3. Нажмите **Тестировать** для проверки настроек
4. Если тест успешен, нажмите **Сохранить**

### Настройка для Gmail

1. Включите двухфакторную аутентификацию в Google аккаунте
2. Создайте [пароль приложения](https://myaccount.google.com/apppasswords)
3. Используйте этот пароль в поле **SMTP Password**
4. Настройки:
   - Host: `smtp.gmail.com`
   - Port: `587`
...
1. Настройки:
   - Host: `smtp.mail.ru`
   - Port: `465`
   - User: ваш Mail.ru email
   - Password: пароль от почты

### Email уведомления

После настройки SMTP система будет автоматически отправлять:

- ✉️ **Покупателям**: подтверждение покупки с ссылкой на скачивание
- ✉️ **Продавцам**: уведомления о новых продажах
- ✉️ **Администраторам**: уведомления о новых товарах, запросах на выплату и отчетах
