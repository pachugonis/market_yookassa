...
export type PaymentProviderId = "YOOKASSA" | "CLOUDPAYMENTS" | "BTCPAY"

/**
 * В чём площадка получает выручку по сделке. Рубли живут на внутреннем
 * балансе в копейках, биткоин — в сатоши: он фиксируется в момент
 * оплаты и по курсу больше не пересчитывается.
 */
export type SettlementAsset = "RUB" | "SATS"
...
export interface ProviderPayment {
  ...
  /** Сколько сатоши получено, если платёж криптовалютный. */
  amountSats?: number | null
  /** Курс на момент оплаты, рублей за 1 BTC. */
  rateRub?: number | null
}
...
export interface PaymentGateway {
  ...
  /**
   * В чём приходит выручка. От этого зависит, какой баланс продавца
   * пополняется при подтверждении сделки: рублёвый или биткоиновый.
   */
  readonly settlementAsset: SettlementAsset

  /**
   * Умеет ли провайдер возвращать деньги покупателю сам. Биткоин — нет:
   * полученную транзакцию не отменить, а обратный перевод требует
   * адреса покупателя. Такие возвраты оформляются задачей администратору,
   * и вызывать `cancel`/`refund` для них бессмысленно.
   */
  readonly refundsAreManual: boolean
