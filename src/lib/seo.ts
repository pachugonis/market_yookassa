...
/**
 * Запасное название площадки: подставляется, пока настроек в базе нет
 * или поле «Название сайта» пустое. Рабочее значение берётся из
 * настроек — `getSiteName()` в `site-settings.ts`.
 */
export const SITE_NAME = "Amazonus"

/** Пояснение после названия в заголовке страницы. */
export const SITE_TAGLINE = "маркетплейс цифровых товаров"

/**
 * Запасное описание площадки: подставляется, пока настроек в базе нет
 * или поле «Описание сайта» пустое.
 */
