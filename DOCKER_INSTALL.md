...
# Сборка обычная, без standalone, поэтому нужны .next, package.json и
# node_modules целиком: приложение поднимается через `next start`.
...
Отдельная сборка для контейнера не нужна: `output: 'standalone'` не
используется — площадка запускается через `next start`, и Next 16 на
сочетание `next start` со standalone ругается прямо при старте.
...
const nextConfig: NextConfig = {
  images: {
    // images.domains устарел в Next 16 — remotePatterns задаёт то же
    // самое, но с явным протоколом.
    remotePatterns: [
      {
        protocol: 'http',
        hostname: 'localhost',
